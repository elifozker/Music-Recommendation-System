...
def dataframex():
    ...
    data_csv.dropna(subset=['popularity'], inplace=True)
...
def main(request):
    if request.method == 'POST':
        ...
        if len(user_input_list) <= 3:
            user_input_dict_list = [{'name': item.split(',')[0].strip(), 'year': int(item.split(',')[1].strip())} for item in user_input_list if ',' in item]
            if all(item['name'] in data['name'].values and item['year'] in data['year'].values for item in user_input_dict_list):
                if user_input_dict_list:
                    ...
                    error_message = "One or more songs in the input do not exist in the dataset."
                    data = {
